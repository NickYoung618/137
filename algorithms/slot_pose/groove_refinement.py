...
DEFAULT_GROOVE_REFINEMENT_CONFIG: dict[str, Any] = {
    "threshold_version": "groove-sidewall-subpixel-v1",
...
def validate_groove_refinement_config(config: dict[str, Any]) -> None:
    ...
    if not isinstance(config["threshold_version"], str) or not config["threshold_version"].strip():
        raise ValueError("groove_refinement.threshold_version must be non-empty")
    for key in (
...
def refine_groove_opening(
    ...
    *,
    ...
    if max_inset <= min_inset:
        return _empty_result(candidate_id, merged, ["insufficient_radial_depth"])
    ...
    if len(radii) < int(merged["min_side_points"]):
        return _empty_result(candidate_id, merged, ["insufficient_radial_support"])
