...
- Modified principles:
  - template placeholders -> I. 规格先行与场景闭环
  - template placeholders -> II. 坐标系与姿态契约明确
  - template placeholders -> III. 质量评估与安全失败
  - template placeholders -> IV. 数据溯源与可复现验证
  - template placeholders -> V. 模块化与集成可控
- Added sections: 工程约束；开发流程与质量门禁
...
不改变语义的澄清升修订版本。每个功能规格和代码评审 MUST 检查 Constitution 合规性；发现
冲突时 MUST 先修订规格、计划或 Constitution，再继续实现。
