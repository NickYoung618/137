...
Mac继续使用`d15703127f9b80351e42d2819f562553003802d2`运行时代码；`e03c262`只包含021文档，
没有改变算法。新增050/080/100共60帧全部为execution、registration、D7、Phi各60/60，无帧级失败。
连同此前010/030，5个独立20帧组共100帧达到100/100技术完成。

| group | D7 sample stdev(px) | D7 range(px) | Phi sample stdev(px) | Phi range(px) |
|---|---:|---:|---:|---:|
| 050 | 0.057682 | 0.198594 | 0.176288 | 0.921465 |
| 080 | 0.061521 | 0.204607 | 0.515701 | 2.676517 |
| 100 | 0.179903 | 0.602547 | 0.334643 | 1.235438 |

唯一权威同图仍PASS：D7绝对误差0.546162px，Phi直径绝对误差0.939461px。新增60帧没有目标真值，
因此100/100只证明这些组上的执行覆盖、技术有效性和组内重复性，不证明它们的绝对准确度。
尤其080组Phi的range=2.676517px应保留为后续监测基线，但本轮没有真值证据支持据此调参。
...
Mac 100帧扩展复验没有推翻条件保留决策，反而确认该运行时版本可作为**初始技术版本**交付：
技术完成率在5个独立组为100/100，权威单图精度门继续PASS。发布边界仍不变化：010的v6 fallback
保持数值有效但证据不可审核；无真值组不得形成精度声明；生产处置仍为`not_evaluated`。
