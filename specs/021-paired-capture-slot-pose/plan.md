...
在020单帧圆定位、暗区、真槽几何、亚像素侧壁和同源性诊断之上增加独立、默认关闭的双帧编排层。每帧仍由既有算法产出完整候选；配对层只处理身份、旋转归一化、一对一匹配、唯一性、无遮挡选择和第二拍后引导。未知旋转参数显式保留UNCONFIRMED，允许诊断但禁止权威姿态/PLC输出。Git外AUTO_预标注工具把019最终选边与020 fixture候选合成一张精简图，避免把完整调试叠加交给现场人工。
...
9. review工具读取显式图片名/manifest与019/020 JSONL，核SHA后只生成原分辨率raw、单张simplified、精简AUTO_LabelMe及RAW/SIMPLIFIED两栏联系表；simplified渲染层不调用完整调试overlay，不画圆、定位框或非最终raw射线。
