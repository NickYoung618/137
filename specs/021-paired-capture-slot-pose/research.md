...
## Decision 12: 先标局部fixture污染子段，再诊断支持点污染

**Decision**: 对145/147保留四项YES和PARTIAL污染的组合语义。现有AUTO墙与端点只作为被复核对象逐点复制，不转为HUMAN真值。下一步只要求人工用left/right专用linestrip描出实际与fixture shadow重叠的墙子段；在子段未到位前，受影响墙、支持点重叠和端点重叠均保持UNCONFIRMED。

**Rationale**: 人工已经确认槽身份、完整可见性和槽肩端点语义，但也确认某些标记线局部落在阴影上。该回答无法区分渲染线延伸穿过阴影与用于拟合的像素支持点被污染。局部子段是作出这一裁决所需的最小新增像素证据。

**Alternatives considered**: 把整条AUTO墙直接升为真值会吞掉污染；要求重画完整槽增加负担且可能引入新的主观误差；直接调同源门或灰度阈值会使用未定位污染进行结果导向调参。三者均拒绝。
