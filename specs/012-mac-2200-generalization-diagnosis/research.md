...
负责人明确授权后实施A1/B1，未实施缺少额外证据支持的C1：

1. reference-phase圆继续受绝对相位峰、对比度、点数、RANSAC残差、极性支持、角覆盖、
   圆心/半径边界约束；其一维有符号分数不再与legacy二维幅值`0.35`交叉比较。
2. legacy magnitude seed仍必须通过未变的`0.35`峰值门和原显著性门；受控phase fallback也
   不能绕过这些门。结果分别记录两套分数、passed状态和采用的score contract。
3. geometry偏差门仍为`0.08`。仅比例离群时保留测量并标记
   `diagnostic_only_unconfirmed`；只有同时出现注册恢复、Phi legacy fallback或D7 v6 fallback
   这类独立风险证据时才硬拒绝。
4. D7没有增加放宽或新hint路径；上游Phi恢复后自然解除连带失效，自身切线质量门保持。

服务器证据：最新唯一真值单图的尺寸7误差为`0.7173203881 px`，Phi直径误差为
`0.1053051052 px`；9帧诊断的500/521/620控制帧状态、变换和测量值均无变化。在服务器阶段，
这些结果只证明单图精度门和小型回归未退化；批量结论由后续Mac T031独立给出。

## 9. 离线双版本审核证据

新增工具只读取old/new batch JSONL和外置原图，不接受目标真值。为防止normal与defective或
diagnostic混算，记录身份固定为`group + 文件名`；组名不同即不配对。默认只渲染注册、尺寸7、
Phi有效状态或失败原因发生变化的帧，也可用显式`--frame`审核控制帧。

外置9帧小样匹配9帧并识别4帧状态变化；显式控制帧620成功生成3072×2048红/青叠加PNG和
LabelMe预测JSON。预测JSON只有old/new尺寸7与Phi四个shape，并明确声明不是零件轮廓或真值。
小样文件保留在仓库外且不提交Git。

## 10. Mac T031最终泛化验收

Mac在提交`2523ada6757e571987f4b40cc9b185acc387f71b`完成严格分组全量复测，退出码0。

### Normal 2000：唯一验收组

| Metric | `79aa6a4`旧基线 | `3ee4b4f`上一候选 | `2523ada`最终 | 对门结果 |
|---|---:|---:|---:|---|
| registrationValid | 1962 | 1985 | 1985 | `1985>=1962` PASS |
| 尺寸7 valid | 1863 | 1772 | 1901 | `1901>=1863` PASS |
| Phi valid | 1922 | 1826 | 1955 | `1955>=1922` PASS |
| technicalComplete | 1863 | 1772 | 1901 | 记录值 |
| execution errors | 0 | 0 | 0 | PASS |

相对`3ee4b4f`，尺寸7与Phi各增加129，registration不变；逐图状态变化恰好129张，全部为
`False→True`，没有`True→False`。相对旧基线，registration、尺寸7、Phi分别为`+23/+38/+33`。

同一统计定义下，normal geometry离群诊断为36张，与上一轮36持平；其中13张同时具有独立风险
证据并按B1策略硬拒绝。这说明B1没有通过删除离群审计或放宽`0.08`取得检测率。

### Defective 200：独立观察组

registration `159`、尺寸7 `108`、Phi `109`，相对`3ee4b4f`状态变化为0。该组没有进入normal
门，也没有用于抵消好样品指标。

### 审核工具全量结果

old/new匹配2200帧，状态变化129，渲染129，unmatched old/new均为0。外置运行目录和生成物只作
事实引用，不进入Git。结合单图尺寸7误差`0.717320388069254 px`、Phi直径误差
`0.10530510518583469 px`，T031的检测率、无新增回归、比例离群和单图精度门全部通过。
