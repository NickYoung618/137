...
1. `Phi12.2-outer-left`：大圆外轮廓左侧可见弧，`linestrip`；
2. `Phi12.2-outer-right`：大圆外轮廓右侧可见弧，`linestrip`；
3. 不标顶部小耳连接区和底部窄颈连接区，不补完整圆；
4. `7-boundary-A`、`7-boundary-B`：窄颈两条真实平行直边，各用有限`line`或`linestrip`；
5. `7`：两条边之间的公法线尺寸连接线。
...
- `prediction:Phi12.2:arc:reference_left:N` / `reference_right:N` (`linestrip`)
- `prediction:7:boundary:A`、`:B`
- `prediction:7:dimension`

若某侧证据未通过，工具不外推该侧轮廓；数值有效性、证据完整性和失败原因分别保留。输出目录
