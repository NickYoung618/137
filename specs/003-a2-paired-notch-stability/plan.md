...
在现有只读历史A端面适配链上做最小增量：继续使用已有圆心、尺度、极坐标和
polar配准，复用指纹锁定的gyj外缘交点/稳健拟圆核心，再做外圆质量门控、全暗区候选、
单帧几何凹槽识别和通用datum/target角色分配。旧单notch与paired路径仅作不回退对照；
外圆质量、凹槽数量/唯一性、环形边界、角色唯一性或业务语义任一未通过时fail-closed。
结果保留v2必填字段，只扩展可忽略的diagnostics。外置Manifest、LabelMe人工圆truth、
角度truth CSV、环形残差和正常/坏图分报告为Mac真实A2批量验收提供可复现入口。
...
2. **配准圆与物理外圆分离**：历史配准圆只作搜索初值；每条径向边缘决策和圆拟合直接调用指纹锁定的gyj `outer_boundary_edge_point`/`robust_fit_circle`，本项目只叠加点数、角覆盖、内点率、残差和先验偏移门控。
...
## 2026-08-14 Physical-Circle Truth Gate

- gyj外圆输出是待测结果，不是truth；两个相关算法的互相一致也不能代替外部真值。
- 物理外圆truth必须由人工在原始BMP上用LabelMe独立画圆，固定标签并由不同人复核，同时记录原图/标注哈希。
- 两点LabelMe圆可用于算法版本比较和系统偏差排查；它不自动构成亚像素计量标定。

## 2026-08-15 Manual Open-Groove Review
