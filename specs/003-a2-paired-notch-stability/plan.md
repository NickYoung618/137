...
4. **通用角色分配为主**：对合格候选枚举datum/target的不重复分配，用可配置方位窗口评分；候选总数可大于角色数。
   仅当最佳方案与次优差距达标时接受唯一角色。
5. **datum和夹角显式化**：datum可配置为单候选射线或对置候选轴；同时输出顺时针有向角与最小夹角，不默认其业务用途。
6. **paired仅兼容**：保留原有两两组合、中心线及polar一致性回归，但不再作为A2角色真值或主数据模型。
