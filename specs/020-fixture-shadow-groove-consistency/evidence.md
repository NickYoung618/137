...
用户补充观察：两处固定装置阴影在相机坐标的位置大致固定/相近，但拍摄角度、透视和照明可能令两者的灰度、
宽度、轮廓强度和整体外观明显不对称。因此，fixtureShadowEvidence中的两影相似性和配对完整性只应作为
诊断，不得作为固定阴影存在与否或候选有效性的单独硬门；单影缺失也不等于固定阴影假设被推翻。

用户同时观察到固定阴影边界更像沿零件圆周延伸的圆弧，而真实槽更像方形开口，可能具有近直且平行的两侧壁、
槽底/开放边界、角点和明确槽口端点。该观察尚未由结构化LabelMe证明，记录状态为PROVISIONAL_FIELD_HYPOTHESIS。
后续在part-015/019/021各2帧上分别审查局部曲率、圆弧连续性、壁平行性、角点/端点、径向深度、槽宽一致性
和灰度/梯度剖面；不得在标注完成前把它写成生产真值或立即新增/放宽门限。

当前020的source_consistency:edge_contrast_asymmetry比较的是“被选作同一槽开口的两侧壁”，不是强制两处
固定阴影彼此对称。该门已拦截人工确认的part-019混合边，但part-008无真值拒绝仍不可裁决，故代码与0.12门限
本轮均保持不变。
